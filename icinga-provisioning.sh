...
for php_path in $php_path1 $php_path2; do
    if [ "$(grep -c "$mystring"  "$php_path")" -lt 1 ]; then
...
if [ "$(mysqlshow -u root -proot123 icinga | wc -l)" -eq 5 ]; then
    mysql -v -u root -proot123 icinga < \
	  /usr/share/icinga2-ido-mysql/schema/mysql.sql
elif [ "$(mysqlshow -u root -proot123 icinga | wc -l)" -eq 66 ]; then
...
if [ "$(grep -c "$mystring" /etc/postfix/main.cf)" -lt 1 ]; then
...
if [ "$(grep -c "$mystring" "$my_url" )" -lt 1 ]; then
